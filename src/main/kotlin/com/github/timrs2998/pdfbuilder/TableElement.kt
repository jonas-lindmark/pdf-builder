...
/**
 * A [TableElement] is a special container [element][Element] for [rows][RowElement]. Unlike other
...
class TableElement(override val parent: Document) : Element(parent), ContainerChild {
  ...
  override fun instanceHeight(width: Float, startY: Float): Float {
    //        return (header?.height(width, startY) ?: 0f) + rows.fold(0.0f, { sum, row -> sum +
    // row.height(width, startY) })
    var currentY = 0f
    var i = 0
    while (i < rows.size) {
      var row = rows[i]
  ...
  // Since table may span multiple pages, breaking on rows, renderInstance() must handle border,
  // background, ..
  override fun render(
  ...
  override fun renderInstance(
    ...
    val rows: MutableList<RowElement> =
        if (header == null) rows else (mutableListOf(header!!) + rows).toMutableList()
    var i = 0
    while (i < rows.size) {
      var row = rows[i]
    ...
    drawLine(
        document, pdDocument, startX, currentY, endX, currentY, border.bottom, border.bottomColor)
