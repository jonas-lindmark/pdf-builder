...
import se.denacode.pdfcompose.drawLine
...
data class Border(
  ...
  companion object {
    @JvmStatic val ZERO = Border()

    @JvmStatic val HALF = Border(.5f, .5f, .5f, .5f)

    @JvmStatic val ONE = Border(1f, 1f, 1f, 1f)
  }
  ...
  /**
   * Given a box defined by the opposite corners (startX, startY) and (endX, endY), draws the
   * surrounding border.
   */
  fun drawBorder(
      document: Document,
