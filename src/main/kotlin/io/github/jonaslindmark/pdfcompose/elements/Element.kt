...
import io.github.jonaslindmark.pdfcompose.drawBox
import io.github.jonaslindmark.pdfcompose.style.*
import java.awt.Color

/**
 * Everything belonging to a [Document] extends from [Element], including [Document] itself. An
 * [Element] knows its [height][Element.height] and how to [render][Element.render] itself.
 *
 * Unless overridden, the default implementations of [render] and [height] assume that the
 * [element][Element] will not span multiple pages and must fit on a single page. The [TableElement]
 * is an example of an [element][Element] that overrides these methods to span multiple pages.
 *
 * An [Element] can be styled similar to CSS, being assigned a [border], [margin], [padding],
 * [backgroundColor], [fontColor], [fontSize], [horizontalAlignment], and [fontName].
 *
 * Most styles will "cascade" or be inherited by children. The only styles that are not inherited
 * are [border], [margin], and [padding].
 */
